...
const PokemonList: FC = () => {
  ...
  return (
    <div className={styles["pokemon-list"]}>
      {count > 1 && (
        <p>
          Showing results {pokemonList.indexOf(pokemonList[0]) + 1 + offsetNum}{" "}-{" "}
          {pokemonList.indexOf(pokemonList[pokemonList.length - 1]) +
            1 + offsetNum}{" "}out of {count} total results
        </p>
      )}
      <SearchInput />
      {count === 0 ? (
